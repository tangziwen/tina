	...
	while ( 1 )
		{
			...
			switch ( t_k.type )
				...
				case TOKEN_TYPE_API:
				{
					...
					if(next.type==TOKEN_TYPE_OP&& next.content[0]=='(')
					...
					else//退化成一个函数指针
						{
							...
							printf("the api function pointer %d\n",a.content.func.func_type);
							put_num_to_RPN ( a );
						}
				}
				break;
				...
				//字符串如果是 ".XXX("形式,则为成员函数调用,我们要加以区分
				//
				case TOKEN_TYPE_STR:
				{
					//瞻前顾后,判断是否为函数的调用
					if(RPN_GetStackTopType()==ELEMENT_OP && GetRPNStackTop()->op=='.')
						{
							int test_pos = *pos;
							//试探读取下一个字符,看是否是左括号,如果是的话,那么就是一个普通的
							//函数,否则就退化一个函数指针.
				...
				case TOKEN_TYPE_RIGHT_PARENTHESIS:
				{
					...
					if ( i==0&&
						...
						{
							Element * top= GetRPNStackTop();
							//top->var_value.content.func.args=pop_arg_count();
							switch(top->type)
								{
								case ELEMENT_API:

									if ( top->var_value.content.func.args>0 )
										{
											Var nul_var;
											nul_var.content.type=VAR_TYPE_NULL;
								...
								case ELEMENT_FUNC:
								{
									Function * top_func=func_get_by_index ( top->index );
									if ( top->var_value.content.func.args>0 )
										{
											Var nul_var;
											nul_var.content.type=VAR_TYPE_NULL;
			...
			for ( i=0; i<=RPN_QueueIndex; i++ )
				{
					switch ( RPN_Queue[i].type )
						...
						case ELEMENT_API:
						{
							...
							//把结果加入到中间语言执行序列中去
							tmp_index++;
							B.value.content.type=VAR_TYPE_NULL;
							B.type=ELEMENT_NUM;
