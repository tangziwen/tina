...
void token_get ( int * pos,TokenInfo * t_k )
{
	...
	if ( is_op ( buffer[postion] ) )
		{
			...
			//考虑出现负常量，如-123.123
			if( pre_tk_type!=TOKEN_TYPE_NUM&&pre_tk_type!= TOKEN_TYPE_FUNC&& pre_tk_type!=TOKEN_TYPE_API&& pre_tk_type!=TOKEN_TYPE_STR&&buffer[postion]=='-')
				{
	...
	if ( isalpha ( buffer[postion] ) )
	...
	else
		{
			...
			if ( buffer[postion]=='"' )
				{
					//从本质上点运算符的右操作数是字符串(如A.XXXX中的XXXX其实被解释器视为字符串)
					//,但是我们为了风格的统一,我们不允许如A."XXXX"这样写
					if(pre_is_dot)
						{
							printf("a string next to the dot operator is illegal!\n");
							exit(0);
						}
					char * tmp_str;
					postion++;
